...
inline static std::vector<Projectile> MakeMultipleProjectiles(
	const Vector2& pos, float speed, float spreadAngleDeg = 45.0f)
{
	std::vector<Projectile> result;
	int numProjectiles = 3;
	float baseAngle = -90.0f; // w górę
	float startAngle = baseAngle - spreadAngleDeg / 2.0f;
	float angleStep = spreadAngleDeg / (numProjectiles - 1);

	for (int i = 0; i < numProjectiles; ++i) {
		float angle = startAngle + i * angleStep;
		float rad = angle * (PI / 180.0f);
		Vector2 vel = { cosf(rad) * speed, sinf(rad) * speed };
		result.emplace_back(pos, vel, 10, WeaponType::SHOTGUN);
	}
	return result;
}
				...
				if (player->IsAlive() && IsKeyDown(KEY_SPACE)) {
					...
					while (shotTimer >= interval) {
						...
						if (currentWeapon == WeaponType::SHOTGUN) {
							auto spreadProjectiles = MakeMultipleProjectiles(p, projSpeed);
							projectiles.insert(projectiles.end(), spreadProjectiles.begin(), spreadProjectiles.end());
						}
						else {
